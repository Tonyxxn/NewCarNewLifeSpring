...
public interface IPropietarioRepository extends JpaRepository<Propietario, Long> {
	...
	@Query(value = "select o.n_propietario, count(v.id_vehiculo)"
			+ "	from Propietario o join vehiculos v on v.id_propietario=o.cpropietario"
			+ "	group by o.n_propietario", nativeQuery = true)
